...
import icbmrl.core.common.implement.IChunkLoadHandler;
...
import icbmrl.explosion.explosive.BlockExplosive;
import icbmrl.explosion.explosive.Explosive;
import icbmrl.explosion.explosive.ExplosiveRegistry;
import icbmrl.explosion.explosive.ItemBlockExplosive;
import icbmrl.explosion.items.ItemBombCart;
import icbmrl.explosion.items.ItemDefuser;
import icbmrl.explosion.items.ItemGrenade;
import icbmrl.explosion.items.ItemLaserDesignator;
import icbmrl.explosion.items.ItemMissile;
import icbmrl.explosion.items.ItemRadarGun;
import icbmrl.explosion.items.ItemRemoteDetonator;
import icbmrl.explosion.items.ItemRocketLauncher;
import icbmrl.explosion.machines.BlockICBMMachine;
import icbmrl.explosion.machines.BlockICBMMachine.MachineData;
import icbmrl.explosion.machines.BlockMissileAssembler;
import icbmrl.explosion.machines.ItemBlockMachine;
import icbmrl.explosion.machines.ItemBlockMissileAssembler;
import icbmrl.explosion.potion.ContagiousPoison;
import icbmrl.explosion.potion.PoisonContagion;
import icbmrl.explosion.potion.PoisonFrostBite;
import icbmrl.explosion.potion.PoisonToxin;

@Mod(modid = ICBMExplosion.ID, name = ICBMExplosion.NAME, version = ModInfo.VERSION, dependencies = "required-after:ICBM;after:ICBM|Sentry")
...
{
    public static final String NAME = ModInfo.NAME + " Explosion";
    public static final String ID = ModInfo.NAME + "|Explosion";
    public static final String CHANNEL = ModInfo.NAME + "|E";
    ...
    public void postInit(FMLPostInitializationEvent event)
    {
        ...
        // EMP Tower
        RecipeUtility.addRecipe(new ShapedOreRecipe(new ItemStack(blockMachine, 1, 10), new Object[] { "?W?", "@!@", "?#?", '?', UniversalRecipe.PRIMARY_PLATE.get(), '!', UniversalRecipe.CIRCUIT_T3.get(), '@', UniversalRecipe.BATTERY_BOX.get(), '#', UniversalRecipe.MOTOR.get(), 'W', UniversalRecipe.WIRE.get() }), "EMP Tower", ModMetadata.CONFIGURATION, true);
        // Cruise Launcher
        ...
        for (Explosive explosive : ExplosiveRegistry.getExplosives())
        {
            explosive.init();
            // Missile
            RecipeUtility.addRecipe(new ShapelessOreRecipe(new ItemStack(itemMissile, 1, explosive.getID()), new Object[] { new ItemStack(itemMissile, 1, Explosive.missileModule.getID()), new ItemStack(blockExplosive, 1, explosive.getID()) }), explosive.getUnlocalizedName() + " Missile", ModMetadata.CONFIGURATION, true);
            if (explosive.getTier() < 2)
            {
                // Grenade
                RecipeUtility.addRecipe(new ShapedOreRecipe(new ItemStack(itemGrenade, 1, explosive.getID()), new Object[] { "?", "@", '@', new ItemStack(blockExplosive, 1, explosive.getID()), '?', Item.silk }), explosive.getUnlocalizedName() + " Grenade", ModMetadata.CONFIGURATION, true);
            }
            if (explosive.getTier() < 3)
            {
                // Minecart
                RecipeUtility.addRecipe(new ShapedOreRecipe(new ItemStack(itemBombCart, 1, explosive.getID()), new Object[] { "?", "@", '?', new ItemStack(blockExplosive, 1, explosive.getID()), '@', Item.minecartEmpty }), explosive.getUnlocalizedName() + " Minecart", ModMetadata.CONFIGURATION, true);
            }
        }
    }
