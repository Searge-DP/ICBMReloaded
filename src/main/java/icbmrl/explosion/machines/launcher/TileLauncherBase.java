package icbmrl.explosion.machines.launcher;
...
public class TileLauncherBase extends TileExternalInventory implements IPacketReceiver, ILauncherContainer, IRotatable, ITier, IMultiBlock, IBlockActivate
{
    ...
    public boolean shiTaiYuan(Vector3 target)
    {
        ...
        if (this.tier == 0)
        {
            if (Vector3.distance(new Vector3(this.xCoord, 0, this.zCoord), new Vector3(target.x, 0, target.z)) < InitModMetadata.DAO_DAN_ZUI_YUAN / 10)
            {
                return false;
            }
        }
        ...
        {
            if (Vector3.distance(new Vector3(this.xCoord, 0, this.zCoord), new Vector3(target.x, 0, target.z)) < InitModMetadata.DAO_DAN_ZUI_YUAN / 5)
            {
                return false;
            }
        }
        ...
        {
            if (Vector3.distance(new Vector3(this.xCoord, 0, this.zCoord), new Vector3(target.x, 0, target.z)) < InitModMetadata.DAO_DAN_ZUI_YUAN)
            {
                return false;
            }
        }
