...
public class ItemRocketLauncher extends ItemICBMElectrical
{
                        ...
                        if (inventoryStack.getItem() instanceof ItemMissile)
                        {
                            ...
                            if (ex instanceof Explosion)
                            {
                                ...
                                if (((Explosion) ex) != null && !evt.isCanceled())
                                {
                                    ...
                                    if (((Explosion) ex).getTier() <= InitModMetadata.MAX_ROCKET_LAUCNHER_TIER && ((Explosion) ex).isCruise())
                                    {
    ...
    {
        String str = LanguageUtility.getLocal("info.rocketlauncher.tooltip").replaceAll("%s", String.valueOf(InitModMetadata.MAX_ROCKET_LAUCNHER_TIER));
        list.add(str);
