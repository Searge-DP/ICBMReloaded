...
public class BlastRedmatter extends Blast
{
    ...
    public void doExplode()
    {
        ...
        {
            this.world().playSoundEffect(position.x + (Math.random() - 0.5) * radius, position.y + (Math.random() - 0.5) * radius, position.z + (Math.random() - 0.5) * radius, ModInfo.PREFIX + "collapse", 6.0F - this.world().rand.nextFloat(), 1.0F - this.world().rand.nextFloat() * 0.4F);
        }

        this.world().playSoundEffect(position.x, position.y, position.z, ModInfo.PREFIX + "redmatter", 3.0F, (1.0F + (this.world().rand.nextFloat() - this.world().rand.nextFloat()) * 0.2F) * 1F);
    }
        ...
        if (Vector3.distance(new Vector3(entity.posX, entity.posY, entity.posZ), position) < 4)
        {
            ...
            else
            {
                if (entity instanceof EntityExplosion)
                {
                    ...
                    {
                        this.world().playSoundEffect(position.x, position.y, position.z, ModInfo.PREFIX + "explosion", 7.0F, (1.0F + (this.world().rand.nextFloat() - this.world().rand.nextFloat()) * 0.2F) * 0.7F);
