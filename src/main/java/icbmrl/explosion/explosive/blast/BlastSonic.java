...
import icbmrl.explosion.explosive.thread.ThreadLargeExplosion.IThreadCallBack;
...
public class BlastSonic extends Blast
{
    ...
    public void doPreExplode()
    {
        ...
        if (this.hasShockWave)
        {
            this.world().playSoundEffect(position.x, position.y, position.z, ModInfo.PREFIX + "hypersonic", 4.0F, (1.0F + (this.world().rand.nextFloat() - this.world().rand.nextFloat()) * 0.2F) * 0.7F);
        }
        else
        {
            this.world().playSoundEffect(position.x, position.y, position.z, ModInfo.PREFIX + "sonicwave", 4.0F, (1.0F + (this.world().rand.nextFloat() - this.world().rand.nextFloat()) * 0.2F) * 0.7F);
        }
    }
