...
public class BlastSky extends BlastBeam
{
    ...
    {
        super.doExplode();
        this.world().playSoundEffect(position.x, position.y, position.z, ModInfo.PREFIX + "redmatter", 4.0F, 0.8F);
    }
    ...
    public void doPostExplode()
    {
        ...
        if (!this.world().isRemote)
        {
            if (this.canFocusBeam(this.world(), position) && this.thread.isComplete)
            {
                ...
                this.world().playSoundEffect(position.x + 0.5D, position.y + 0.5D, position.z + 0.5D, ModInfo.PREFIX + "redmatter", 6.0F, (1.0F + (world().rand.nextFloat() - world().rand.nextFloat()) * 0.2F) * 1F);
            }
