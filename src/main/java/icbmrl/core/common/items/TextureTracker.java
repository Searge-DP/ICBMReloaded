	...
	public void updateAnimation()
	{
		...
		if (world != null)
		{
			...
			if (itemStack != null)
			{
				...
				{
					Entity trackingEntity = ((ItemTracker) itemStack.getItem()).getTrackingEntity(FMLClientHandler.instance().getClient().theWorld,
							itemStack);
		...
		for (i = (int) ((this.currentAngle / (Math.PI * 2D) + 1.0D) * this.framesTextureData.size())
				% this.framesTextureData.size(); i < 0; i = (i + this.framesTextureData.size()) % this.framesTextureData.size())
		{
			;
		}
		...
		{
			this.frameCounter = i;
			// TextureUtil.uploadTextureSub(i, (int[])
			// this.framesTextureData.get(this.frameCounter), this.width,
			// this.height, this.originX, this.originY,
			// false, false, rotated);
		}
	}
}
