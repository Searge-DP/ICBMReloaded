...
{
	// TODO Subsume this into RenderMinedBlock
	...
	public void doRenderGravityBlock(EntityFlyingBlock entity, double x, double y, double z, float par8, float par9)
	{
		...
		this.renderManager.worldObj = world;

		if (block == Blocks.dragon_egg || block == Blocks.grass || block == Blocks.fence || block == Blocks.wheat || block == Blocks.leaves
				|| block == Blocks.unlit_redstone_torch || block == Blocks.torch || block == Blocks.redstone_torch || block == Blocks.tallgrass
				|| block == Blocks.vine || block == Blocks.planks || block == Blocks.bookshelf || block == Blocks.pumpkin)
		{
			Tessellator tessellator = Tessellator.instance;
			tessellator.startDrawingQuads();
			tessellator.setTranslation((-MathHelper.floor_double(entity.posX)) - 0.5F, (-MathHelper.floor_double(entity.posY)) - 0.5F,
					(-MathHelper.floor_double(entity.posZ)) - 0.5F);
			this.blockRenderer.renderBlockByRenderType(block, MathHelper.floor_double(entity.posX), MathHelper.floor_double(entity.posY),
		...
		{
			this.renderBlockGravity(block, entity.metadata, this.blockRenderer);
		}
